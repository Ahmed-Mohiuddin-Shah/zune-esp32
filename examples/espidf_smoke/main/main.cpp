...
public:
    void onUserCreate() override {
        ...
        // ponytail: tiny Clay — FB already claimed ~150KB of the big DRAM slab
        if (!clay_.init(ui::ClayHostConfig{screenWidth_, screenHeight_, 48 * 1024, 64})) {
            hal::Log::error("Smoke", "Clay init failed");
            return;
        }

        hal::TouchCalData cal{};
        if (hal::TouchCalib::load(&cal)) {
            hal::TouchCalib::apply(cal);
            hal::Log::info("Smoke", "touch cal loaded from NVS");
        } else if (renderer_ && display_) {
...
private:
    ...
    void drawOptions() {
        ...
        if (ui::drawButton("o_tcal", "[ CALIBRATE TOUCH ]")) {
            if (renderer_ && display_ && hal::TouchCalib::run(*renderer_, *display_)) {
                setToast("Touch cal saved");
            } else {
                setToast("Touch cal failed");
            }
        }
